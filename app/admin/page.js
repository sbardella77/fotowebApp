      ...
      <div className="space-y-3 p-3">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="truncate text-sm font-medium text-white">{photo.originalName}</p>
          ...
          <div className="mx-auto flex max-w-6xl flex-col gap-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant="outline" className="rounded-full border-white/[0.07] bg-[#141C2E] px-3 py-1 font-mono text-[0.65rem] uppercase tracking-[0.1em] text-muted-foreground">
                  {authState.source ? `${authState.source} auth` : 'not configured'}
                </Badge>
                <Button asChild variant="outline" className="rounded-full border-white/[0.07] bg-[#141C2E] hover:bg-[#111827] hover:text-foreground">
            ...
            {message ? (
              <div className="rounded-xl border border-white/[0.07] bg-[#141C2E] p-4 text-sm font-light text-muted-foreground">
                {message}
              </div>
            ) : null}
          </div>
        ...
        <div className="rounded-2xl border border-white/[0.07] bg-[#141C2E] shadow-card">
          <div className="p-5 sm:p-6">
            ...
            <div className="mt-4 space-y-4">
              {authState.loading ? (
                <div className="flex items-center gap-2 rounded-xl border border-white/[0.07] bg-[#0D1220] p-4 text-sm font-light text-muted-foreground">
                  <Loader2 className="h-4 w-4 animate-spin" />
                  Loading admin state...
                </div>
              ) : authState.authenticated ? (
                <div className="flex items-start gap-3 rounded-xl border border-white/[0.07] bg-[#0D1220] p-4 text-sm font-light text-muted-foreground">
                ...
                <div className="space-y-3">
                  <div className="space-y-2">
                    ...
                    <Input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Enter admin password" className="border-white/[0.07] bg-[#0D1220] text-foreground placeholder:text-muted-foreground/60" />
                  </div>
                  {!authState.configured ? (
                    <Button className="w-full glow-blue" disabled={busy.auth} onClick={() => submitAuth('setup')}>
                      {busy.auth ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Create admin password'}
                    </Button>
                  ) : (
                    <Button className="w-full glow-blue" disabled={busy.auth} onClick={() => submitAuth('login')}>
          ...
          <div className="rounded-2xl border border-white/[0.07] bg-[#141C2E] shadow-card">
            <div className="p-5 sm:p-6">
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                <Input value={newEventName} onChange={(event) => setNewEventName(event.target.value)} placeholder="Wedding Reception" className="border-white/[0.07] bg-[#0D1220] text-foreground placeholder:text-muted-foreground/60" />
                <Button disabled={!authState.authenticated || busy.create} onClick={createEvent} className="glow-blue">
            ...
            <div className="rounded-2xl border border-white/[0.07] bg-[#141C2E] shadow-card">
              <div className="p-5 sm:p-6">
                ...
                <div className="mt-4 space-y-3">
                  {events.length === 0 ? (
                    <div className="flex flex-col items-center rounded-xl border border-dashed border-white/[0.07] bg-[#0D1220] p-6 text-center">
                    ...
                    events.map((event) => (
                      <button
                        key={event.id}
                        className={`w-full rounded-xl border p-3 text-left transition ${selectedSlug === event.slug ? 'border-primary bg-primary/[0.08]' : 'border-white/[0.07] bg-[#0D1220] hover:border-white/[0.12]'}`}
              ...
              <div className="p-5 sm:p-6">
                ...
                <div className="mt-4 space-y-4">
                  {busy.detail && !selectedEvent ? (
                    ...
                    <>
                      <div className="rounded-xl border border-white/[0.07] bg-[#0D1220] p-4">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          <div>
                            <span className="font-mono text-[0.65rem] uppercase tracking-[0.1em] text-primary">Moderating</span>
                            <p className="mt-0.5 font-display text-base font-bold tracking-tight text-white">{selectedEvent.name}</p>
                      ...
                      {photos.length === 0 ? (
                        <div className="flex flex-col items-center rounded-xl border border-dashed border-white/[0.07] bg-[#0D1220] p-8 text-center">
