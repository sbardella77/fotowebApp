...
describe('STEP 4.7: atomic finalization for checkout.session.completed / extra_event canonical credit path', () => {
  ...
  it('H: legacy extra_event (no pendingCheckout row) grants credit outside any transaction — does not use the atomic tagged path', async () => {
    ...
    // Legacy path is not transactional: no $transaction call at all.
    expect(prisma.$transaction).not.toHaveBeenCalled()
    ...
    // UpsellEvent is written directly against the root client in the
    // legacy path, not via a tx — already asserted indirectly by
    // $transaction never being called, but also check it landed.
    expect(prisma.upsellEvent._rows).toHaveLength(1)

    // Finalized via the wrapper's standalone markProcessed on the legacy
    // 200 response, not via a RECEIPT_ALREADY_FINALIZED tagged outcome.
    const receipt = await prisma.stripeWebhookEvent.findUnique({ where: { eventId: stripeEventId } })
    expect(receipt.status).toBe('PROCESSED')
    expect(markStripeWebhookEventProcessed).toHaveBeenCalledTimes(1)
  })
