...
|---|---|---|---|---|
| `wedding-couple.jpg` | SOURCE METADATA MISSING | SOURCE METADATA MISSING | SOURCE METADATA MISSING | Unsplash/Pexels standard free license (provider not recorded) |
| `wedding-guests.jpg` | SOURCE METADATA MISSING | SOURCE METADATA MISSING | SOURCE METADATA MISSING | Unsplash/Pexels standard free license (provider not recorded) |
...
| `wedding-detail.jpg` | SOURCE METADATA MISSING | SOURCE METADATA MISSING | SOURCE METADATA MISSING | Unsplash/Pexels standard free license (provider not recorded) |
| `wedding-group.jpg` | SOURCE METADATA MISSING | SOURCE METADATA MISSING | SOURCE METADATA MISSING | Unsplash/Pexels standard free license (provider not recorded) |
| `wedding-toast.jpg` | SOURCE METADATA MISSING | SOURCE METADATA MISSING | SOURCE METADATA MISSING | Unsplash/Pexels standard free license (provider not recorded) |
...
**`SOURCE METADATA MISSING — REPLACEMENT REQUIRED BEFORE FINAL BRAND ASSET FREEZE`**
...
per-image manifest into the repository. The files themselves carry no embedded
EXIF/attribution metadata (confirmed via `sips`/`mdls` — both stock providers
strip this on download), so the original source page, photographer name, and
provider for these 10 files cannot be reconstructed after the fact and are not
being guessed here. Before these leave placeholder status and become permanent
brand assets, each one needs to be re-sourced (or the original selection
re-found and logged) so this table can be completed with real data.
...
photography. Recommended before any brand/marketing asset freeze:
1. Fill in the remaining "SOURCE METADATA MISSING" rows in the Originals
   table above (now 5 wedding-set originals, down from 9 at the start of
   Phase 3 — see Phase 3B/3C/3D), or replace those photos with newly-sourced
   ones logged at selection time.
2. Resolve the `planner-supervision` gap noted under Phase 3D above.
