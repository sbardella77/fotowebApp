...
export function resolveEventRetentionState({
  ...
  const canceledAt = toDate(ownerSubscriptionCanceledAt)
  if (gracePeriodUntil || canceledAt) {
    // gracePeriodUntil is already the absolute grace deadline; otherwise
    // fall back to 90 days from the cancellation date.
    const until = gracePeriodUntil || addDays(canceledAt, 90)
