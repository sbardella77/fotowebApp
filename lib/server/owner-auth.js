...
const getSessionSecret = async () => {
  ...
  // No runtime fallback: the owner session secret must never be derivable
  // from ADMIN_SESSION_SECRET, ADMIN_PASSWORD, or admin credential material.
  throw new Error('OWNER_SESSION_SECRET is not set. Configure it explicitly; it must be independent from admin credentials.')
}
