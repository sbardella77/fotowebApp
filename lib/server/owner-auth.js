...
const getSessionSecret = () => {
  return process.env.OWNER_SESSION_SECRET || process.env.ADMIN_SESSION_SECRET || 'snaprooms-owner-session-fallback'
}
...
export const createOwnerSessionToken = (email) => {
  const payload = {
...
export const verifyOwnerSessionToken = (token) => {
  if (!token) {
    return null
  }
  ...
  const expectedSignature = createHmac('sha256', getSessionSecret()).update(encodedPayload).digest('hex')
