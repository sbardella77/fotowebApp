...
import { checkTargetIdentity, ENV_CONFIG } from '@/lib/server/backfill-target-identity'
...
const KNOWN_FLAGS = new Set(['--apply', '--env', '--confirm-target', '--expected-visible', '--expected-missing', '--expected-reconcile'])
const INTEGER_FLAG_PATTERN = /^(--expected-visible|--expected-missing|--expected-reconcile)=(.*)$/
const STRING_FLAG_PATTERN = /^(--env|--confirm-target)=(.*)$/
 ...
 *
 * --env=preview|production (Operator Target Safety) is ALWAYS required,
 * dry-run and --apply alike, so every invocation — including a dry-run
 * report — is unambiguous about which environment it describes. No
 * environment inference is ever performed.
 *
 * --confirm-target=preview|production replaces the old, environment-blind
 * --confirm-production flag: it is required only for --apply, and MUST
 * equal --env's value exactly. This makes an accidental Production
 * confirmation structurally harder than a bare boolean flag would: the
 * operator must type the target's name a second time, and any mismatch
 * (including a typo) fails closed rather than silently defaulting to
 * either environment.
 *
...
export function parseArgs(argv) {
  ...
  for (const arg of argv) {
    ...
    const stringMatch = STRING_FLAG_PATTERN.exec(arg)
    if (stringMatch) {
      const [, name, rawValue] = stringMatch
      if (!VALID_ENVS.has(rawValue)) {
        return { error: `Invalid value for ${name}: must be 'preview' or 'production'` }
      }
      if (name === '--env') env = rawValue
      else confirmTarget = rawValue
      continue
    }

    const intMatch = INTEGER_FLAG_PATTERN.exec(arg)
    if (intMatch) {
      const [, name, rawValue] = intMatch
      if (!/^\d+$/.test(rawValue)) {
  ...
  if (!env) {
    return { error: '--env=preview|production is required' }
  }

  if (apply) {
    if (!confirmTarget) return { error: '--apply requires --confirm-target=preview|production' }
    if (confirmTarget !== env) {
      return { error: `--confirm-target (${confirmTarget}) does not match --env (${env}); refusing to apply` }
    }
  ...
  return { apply, env, confirmTarget, expectedVisible, expectedMissing, expectedReconcile }
}
 ...
 *
 * Target-identity is checked BEFORE any DB/Blob call this function would
 * otherwise make (before even obtaining `prisma` for the inventory), for
 * BOTH dry-run and --apply — see checkTargetIdentity in
 * lib/server/backfill-target-identity.js. A dry-run against the wrong
 * environment produces a misleading report, not a mutation, but there is no
 * reason to allow even that: identity is orthogonal to --apply and is
...
export async function main(argv, dependencies) {
  ...
  if (!identity.ok) {
    logger.error('[backfill] TARGET_IDENTITY=FAIL')
    logger.error(`[backfill] REASON=${identity.reason}`)
    for (const [key, value] of Object.entries(identity)) {
      if (key === 'ok' || key === 'reason') continue
      logger.error(`[backfill] ${key}=${value}`)
    }
    return { exitCode: 1, mode: 'identity-check', identity }
  }
  ...
  logger.log(
    [
      ...
      `ENV=${identity.env}`,
      `DB=${identity.databaseName}`,
      `DB_ROLE=${identity.databaseRole}`,
      `DB_BRANCH=${identity.databaseBranchEndpoint}`,
      `BLOB_STORE=${identity.blobStoreId}`,
    ].join('\n'),
  )
...
/**
 * The only place real production primitives are wired together — and,
 * per lib/server/backfill-target-identity.js's own doc comment, the one
 * deliberate exception to this module never reading process.env: raw
 * DATABASE_URL/BLOB_READ_WRITE_TOKEN are read here, once, purely to hand
 * their non-secret parsed identity to checkTargetIdentity — never logged,
 * never otherwise used. queryLiveDatabaseIdentity reuses the SAME cached
 * getPrismaClient() singleton main()'s own `getPrisma()` call resolves to
 * (see lib/server/prisma-client.js), so this opens no second connection.
 */
export function createRealDependencies() {
  return {
    ...
    queryLiveDatabaseIdentity: async () => {
      const prisma = await getPrismaClient()
      if (!prisma) throw new Error('Prisma client unavailable')
      const rows = await prisma.$queryRaw`SELECT current_database() AS "databaseName", current_user AS "databaseRole"`
      return rows[0]
    },
