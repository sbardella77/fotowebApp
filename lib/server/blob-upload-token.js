 ...
 */
export function parseCanonicalSessionPayload(value) {
 ...
 *
 * The installed @vercel/blob client does not include callbackUrl in the
 * generate-client-token request payload at all — the SDK derives the
 * callback URL itself, server-side, from the incoming request. So an
 * absent/undefined callbackUrl is not a client attempting to control the
 * callback; it is skipped here and left to the SDK's own derivation. If a
 * callbackUrl IS present (any future/alternate caller), it is still
 * validated strictly.
 *
