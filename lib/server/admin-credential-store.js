...
const localAdminCredentialStore = {
  ...
  async setup(password) {
    if (process.env.VERCEL) {
      throw new Error('Local admin credential setup is not supported on Vercel. Keep ADMIN_AUTH_DRIVER=local only with a pre-seeded admin-auth.json file, or migrate admin auth in a later step.')
    }
  ...
  async verify(password) {
    ...
    const candidateHash = await hashPassword(password, localConfig.salt)
...
const prismaAdminCredentialStore = {
  ...
  async verify(password) {
    ...
    const candidateHash = await hashPassword(password, credential.passwordSalt)
