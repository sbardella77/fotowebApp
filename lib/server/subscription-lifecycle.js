...
export function resolveSubscriptionAccessState(owner = {}) {
  ...
  // Portal eligibility is deliberately narrower than premium entitlement:
  // stripeSubscriptionId is the only field that means "a real Stripe
  // subscription object exists for the Customer Portal to manage." Plan
  // labels and prior cancellation history do not — a legacy/inconsistent
  // owner can have plan === 'professional' (and therefore full premium app
  // access via accountPremiumActive above) with no Stripe subscription at
  // all, in which case the portal has nothing to show and must not be
  // offered. stripeSubscriptionId stays populated through every lifecycle
  // state where the portal is actually useful (active, trialing, past_due,
  // unpaid, cancel_at_period_end) — see buildSubscriptionUpdatedData, which
  // never touches it except to null it out on a terminal 'canceled' status
  // — and is nulled by buildSubscriptionDeletedData once the subscription
  // is fully gone, at which point there is nothing left to manage.
  const canManageSubscription = Boolean(stripeSubscriptionId)
...
export function buildPaymentFailedUpdate(invoice = {}) {
  ...
  // Invoice domain only: subscriptionStatus/subscriptionGraceUntil are owned
  // exclusively by the subscription domain (customer.subscription.updated).
  return {
    paymentFailedAt: now,
...
export function buildPaymentSucceededUpdate(invoice = {}) {
  // Invoice domain only: plan/subscriptionStatus/subscriptionGraceUntil/
  // subscriptionCanceledAt are owned exclusively by the subscription domain
...
export function buildSubscriptionUpdatedData({ status, subscription, owner = {} }) {
  ...
  if (ACTIVE_STATUSES.has(effectiveStatus)) {
    // paymentFailedAt/lastPaymentError are invoice-domain fields; subscription
    // domain only owns subscriptionGraceUntil here.
    ...
    // Subscription domain owns grace initialization; paymentFailedAt is
    // invoice-domain-only (set by invoice.payment_failed).
    ...
    if (!isGraceActive(owner.subscriptionGraceUntil)) {
      // Grace has expired; clear the subscription-domain grace field.
      update.subscriptionGraceUntil = null
    }
