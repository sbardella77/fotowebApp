 ...
 *
 * @returns {Promise<{buffer: Buffer, created: boolean}>} `created` is false
 *   only when our own put lost a race to a concurrent producer whose object
 *   is already there — `buffer` is still populated in that case (the
 *   transform is a pure function of the source, so it is what would have
 *   been stored either way), it just wasn't THIS call's write that won.
 */
async function produceAndPersist({ kind, derivativePath, getSourceBuffer, onTransform }) {
  ...
  try {
    ...
    return { buffer: derivativeBuffer, created: false }
  }

  return { buffer: derivativeBuffer, created: true }
}
...
/**
 * Convert a probe-confirmed HIT into this call's return shape.
 *
 * `returnBytes: true` is the existing byte-returning contract
 * (getDisplayDerivative/getBrandedDerivative) — unchanged, still reads and
 * returns the object. `returnBytes: false` is the ensure-only contract
 * (STEP 7.15e): a HIT never fetches derivative bytes at all — the whole
 * reason that mode exists is so a rerun/backfill over many already-generated
 * photos costs one Blob `head` each, not a full download.
 */
async function settleHit(url, returnBytes) {
  if (!returnBytes) return { created: false }
  return readDerivative(url)
}

/** Convert a produce-or-race outcome into this call's return shape. */
function settleProduced({ buffer, created }, returnBytes) {
  return returnBytes ? buffer : { created }
}

/**
 * Resolve a derivative for a photo, transforming at most once.
 *
 * Shared by every derivative kind AND by both the byte-returning and
 * ensure-only entry points, so the single-flight and
 * never-transform-without-the-lock guarantees cannot drift between kinds or
 * between modes. `returnBytes` only ever changes what a HIT or a produced
 * result is converted TO at the very end of each branch — every probe, lock,
 * waiter and produce/put-race decision above that point is identical
 * regardless of mode.
 *
 ...
 * @param {boolean} returnBytes                            true: return derivative bytes (existing contract). false: return {created} only.
 * @returns {Promise<Buffer|{created: boolean}>}
 */
async function resolveOrEnsureDerivative(kind, { photoId, getSourceBuffer, onTransform, now = Date.now }, returnBytes) {
  ...
  //    source fetch (and, in ensure mode, no derivative-bytes fetch either).
  const initial = await probeDerivative(derivativePath)
  if (initial.exists) {
    return settleHit(initial.url, returnBytes)
  }
  ...
  for (;;) {
    ...
    if (outcome === DISTRIBUTED_LOCK_OUTCOME.ACQUIRED) {
      ...
      if (afterLock.exists) {
        return settleHit(afterLock.url, returnBytes)
      }
      const produced = await produceAndPersist({ kind, derivativePath, getSourceBuffer, onTransform })
      return settleProduced(produced, returnBytes)
    }

    if (outcome === DISTRIBUTED_LOCK_OUTCOME.BACKEND_UNAVAILABLE) {
      // Shared across every mode for this lock key: the map holds one
      // promise per key, not per (key, mode) pair, so whichever caller
      // registers the task first determines what every other caller sharing
      // it receives. This is safe today because getDisplayDerivative has
      // zero production callers — a get+ensure race on the same photo in
      // degraded mode cannot occur yet. Revisit if that changes (e.g. the
      // Guest display cutover starts calling getDisplayDerivative for real).
      return runWithLocalSingleFlight(lockKey, async () => {
        ...
        if (beforeLocalTransform.exists) {
          return settleHit(beforeLocalTransform.url, returnBytes)
        }
        const produced = await produceAndPersist({ kind, derivativePath, getSourceBuffer, onTransform })
        return settleProduced(produced, returnBytes)
      })
    ...
    if (polled.exists) {
      return settleHit(polled.url, returnBytes)
    }
 ...
 *
 * Callers get only `buildPath`, `buildLockKey`, `resolve` and `ensure` — the
 * Redis and Blob mechanics stay private to this module.
 *
 * `resolve` and `ensure` (STEP 7.15e) share the exact same probe/lock/waiter/
 * single-flight/produce/put-race orchestration; `ensure` only differs in
 * that a HIT never fetches derivative bytes and a produce returns
 * `{created}` instead of the buffer.
 *
 * @param {object} spec
...
export function createDerivativeKind({
  ...
  const kind = Object.freeze({
    ...
    resolve: (params) => resolveOrEnsureDerivative(kind, params, true),
    ensure: (params) => resolveOrEnsureDerivative(kind, params, false),
  })
  return kind
}
