 ...
 * JPEG quality, or flatten background. The derivative cache (lib/server/
 * download-derivative.js) uses it as part of its storage key, so a change
 * without a bump serves stale bytes forever.
 *
 * Bumped v1 -> v2: replaced the legacy gold badge asset
 * (public/watermark/snaprooms-watermark.png) with the canonical V4 lime
 * mark. Without this bump, any photo already downloaded via the branded
 * single-photo path would keep serving its old-logo derivative from cache
 * indefinitely (derivatives/wm-v1/{photoId}.jpg never gets re-derived).
 */
export const WATERMARK_DERIVATIVE_VERSION = 'v2'
