 ...
 * @param {typeof ensureDisplayDerivative} [params.ensure]  injectable override
 *   of the underlying ensure call (STEP: Display Backfill Status
 *   Reconciliation). Defaults to the real ensureDisplayDerivative; exists so
 *   the batch backfill caller can thread its own injected test double through
 *   this shared helper without vi.mock, keeping the "pure function of
 *   injected dependencies" testing style used throughout
 *   lib/server/display-backfill.js. Never used by route.js's call sites.
 ...
 */
export async function ensurePhotoDisplayDerivativeStatus({ prisma, photo, operation, sourceBuffer, getSourceBuffer, ensure = ensureDisplayDerivative }) {
  try {
