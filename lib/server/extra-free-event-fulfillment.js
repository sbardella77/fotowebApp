...
import { serializeProviderError } from './safe-log.js'
...
export async function fulfillExtraFreeEventCreditCanonical({
  ...
  console.log(`[extra-free-event-fulfillment] Owner ${updatedOwner.id} granted extra free event credit. Total credits: ${updatedOwner.extraEventCredits}`)
...
export async function fulfillExtraFreeEventBuyAndCreate({
  ...
  try {
    ...
    try {
      ...
      console.error('[extra-free-event-fulfillment] Fallback credit grant also failed:', serializeProviderError('db', 'extra_free_event_fallback_credit', fallbackError))
      await sendOpsAlert({
        severity: 'critical',
