...
export const getPrismaClient = async () => {
  ...
  try {
    ...
    if (!prismaClientInstance) {
      ...
      // Trigger ownership repairs asynchronously on first init.
      // We use dynamic imports to avoid circular dependencies.
      const { repairLegacyOwnership } = await import('./repair-legacy-ownership.js')
      repairLegacyOwnership().catch((err) => {
        console.error('[getPrismaClient] Background legacy repair failed:', sanitizeError(err))
      })

      const { repairDuplicateOwnerAssignments } = await import('./owner-resolution.js')
      repairDuplicateOwnerAssignments().catch((err) => {
        console.error('[getPrismaClient] Background duplicate-owner repair failed:', sanitizeError(err))
      })
    }
