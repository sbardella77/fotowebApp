...
export async function getEffectiveEventAccessState(prisma, event) {
  ...
  let ownerSubscriptionStatus = event.ownerSubscriptionStatus ?? null
  let ownerSubscriptionGraceUntil = event.ownerSubscriptionGraceUntil ?? null
  let ownerSubscriptionCanceledAt = event.ownerSubscriptionCanceledAt ?? null

  // Fallback to DB lookup when owner fields are not denormalised
  if ((ownerPlan == null || ownerSubscriptionStatus == null) && event.ownerId && prisma) {
  ...
  return resolveEffectiveEventAccessState({
    ...
    subscriptionCanceledAt: ownerSubscriptionCanceledAt,
  })
}
