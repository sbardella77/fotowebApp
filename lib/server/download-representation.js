...
export async function resolveDownloadRepresentation({
  ...
  // `hasUnbrandedDownloads === false` is the explicit Free/branded state.
  // Treat an omitted field as unbranded for resolver-level tests/callers that
  // only exercise the quality contract; production access objects set it.
  if (access?.hasUnbrandedDownloads === false) {
    return {
      quality,
