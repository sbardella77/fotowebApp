...
export const prismaGalleryRepository = {
  ...
  async getEventPhotosPaginated({ eventId, cursor, take = 24, sort = 'recent' }) {
    const start = Date.now()
    const prisma = await getPrismaClient()
    if (!prisma) {
      throw new Error('Prisma repository requested but DATABASE_URL/client is unavailable')
    ...
    console.log(`[repo:getEventPhotosPaginated] eventId=${eventId} cursor=${cursor} sort=${sort} take=${take} returned=${effectivePhotos.length} total=${total} durationMs=${Date.now() - start}`)
