...
/**
 * Outcomes of an attempt to take a short-lived distributed mutex.
 *
 * BACKEND_UNAVAILABLE must never be conflated with HELD_BY_OTHER: the
 * former means we learned nothing about the lock (Redis is down or not
 * configured) and the caller should degrade to a local strategy, while the
 * latter means another holder genuinely owns it right now.
 */
export const DISTRIBUTED_LOCK_OUTCOME = Object.freeze({
  ACQUIRED: 'ACQUIRED',
  HELD_BY_OTHER: 'HELD_BY_OTHER',
  BACKEND_UNAVAILABLE: 'BACKEND_UNAVAILABLE',
})

let lastLockErrorLogAt = 0

/**
 * Try to take a short-lived distributed mutex via SET key value NX EX.
 *
 * Release is deliberately TTL-only — there is no companion release()
 * function. Callers must be safe against a lock that outlives the work it
 * guarded, which avoids the classic "delete a lock another invocation has
 * since reacquired" hazard entirely.
 *
 * Never throws and never fails closed: any backend problem surfaces as
 * BACKEND_UNAVAILABLE so the caller can pick its own degraded path.
 *
 * @param {string} key
 * @param {number} ttlSeconds
 * @returns {Promise<{outcome: string}>}
 */
export async function tryAcquireDistributedLock(key, ttlSeconds) {
  if (!redisClient) {
    return { outcome: DISTRIBUTED_LOCK_OUTCOME.BACKEND_UNAVAILABLE }
  }
  ...
  try {
    ...
    return {
      outcome: result === 'OK'
        ? DISTRIBUTED_LOCK_OUTCOME.ACQUIRED
        : DISTRIBUTED_LOCK_OUTCOME.HELD_BY_OTHER,
    }
  } catch (error) {
    // Bounded logging, mirroring checkRateLimit's own interval so a Redis
    // outage cannot produce per-request log spam.
    const now = Date.now()
    if (now - lastLockErrorLogAt > REDIS_ERROR_LOG_INTERVAL_MS) {
      console.error('[rate-limit] Redis lock acquisition failed:', error.message)
      lastLockErrorLogAt = now
    }
    return { outcome: DISTRIBUTED_LOCK_OUTCOME.BACKEND_UNAVAILABLE }
  }
}
...
export const ADMIN_LIMITS = {
  ...
  read: { ip: { max: 120, window: 10 * 60 * 1000 }, admin: { max: 200, window: 10 * 60 * 1000 } },
}
