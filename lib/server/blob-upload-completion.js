    ...
    txResult = await prisma.$transaction(async (tx) => {
      ...
      const photo = await tx.photo.create({
        data: {
          ...
          contributorId: claim.session.contributorId || null,
          // Session-authoritative, resolved server-side at init time —
          // never re-derived or accepted from the completion request.
          uploadActorType: claim.session.uploadActorType || null,
