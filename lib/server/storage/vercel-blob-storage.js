...
export const buildPrivateDeliveryBlobPathname = ({ eventSlug, fileName }) => {
  const extension = path.extname(fileName)
  const safeBaseName = sanitizeName(path.basename(fileName, extension))
  return `private-delivery/${eventSlug}/${randomUUID()}-${safeBaseName}${extension}`
}
...
export const vercelBlobStorageDriver = {
  ...
  async initUploadSession({ eventSlug, fileName, directory = 'events' }) {
    const pathname = directory === 'private-delivery'
      ? buildPrivateDeliveryBlobPathname({ eventSlug, fileName })
      : buildBlobPathname({ eventSlug, fileName })
    return {
