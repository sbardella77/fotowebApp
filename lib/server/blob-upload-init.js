...
export async function createServerBoundBlobUploadInit({
  ...
  const databaseSession = await createBlobUploadSession(prisma, {
    ...
    // Only ever populated by the ROOM_PHOTO (gallery) caller — the
    // PRIVATE_DELIVERY/PHOTOGRAPHER_UPLOAD callers do not pass this and it
    // stays null for those sessions (they use PrivateAsset.uploadedByRole
    // instead). See lib/server/gallery-upload-actor.js.
