 ...
 */
export function resolveRoomCreationEntitlement({ ownerPlan, subscriptionStatus, subscriptionGraceUntil, currentRooms, extraEventCredits = 0, freeRoomLimit = FREE_ROOM_LIMIT }) {
  const state = resolveEffectiveEventAccessState({ ownerPlan, subscriptionStatus, subscriptionGraceUntil })
  if (state.canCreateUnlimitedRooms) {
    return {
      ...
      max: Infinity,
      extraEventCredits,
      consumeExtraCredit: false,
      canBuyExtraEvent: false,
      upgradePath: null,
      oneTimePath: null,
    }
  }
  ...
  const safeCredits = Number(extraEventCredits || 0)
  const withinFreeIncludedLimit = safeCurrentRooms < freeRoomLimit
  const hasExtraEventCredits = safeCredits > 0

  if (withinFreeIncludedLimit) {
    return {
      allowed: true,
      current: safeCurrentRooms,
      max: freeRoomLimit,
      extraEventCredits: safeCredits,
      consumeExtraCredit: false,
      canBuyExtraEvent: false,
      upgradePath: null,
      oneTimePath: null,
  ...
  if (hasExtraEventCredits) {
    return {
      allowed: true,
      current: safeCurrentRooms,
      max: freeRoomLimit,
      extraEventCredits: safeCredits,
      consumeExtraCredit: true,
      canBuyExtraEvent: false,
      upgradePath: null,
      oneTimePath: null,
    }
...
export async function checkOwnerRoomCreationEntitlement(prisma, owner) {
  if (!prisma || !owner) {
    return { allowed: false, current: 0, max: 0, extraEventCredits: 0, consumeExtraCredit: false, canBuyExtraEvent: false, reason: 'service_unavailable', upgradePath: null, oneTimePath: null, error: 'Service temporarily unavailable, please retry' }
  }
...
export async function checkRoomUploadEntitlement(prisma, event) {
  if (!prisma || !event) {
    return { allowed: false, current: 0, max: 0, upgradePath: null, error: 'Service temporarily unavailable, please retry' }
  }

  // Resolve owner plan when not already available
  let ownerPlan = event.ownerPlan ?? null
  let ownerSubscriptionStatus = null
  let ownerSubscriptionGraceUntil = null
  if (ownerPlan == null && event.ownerId) {
    const owner = await prisma.owner.findUnique({
      where: { id: event.ownerId },
      select: { plan: true, subscriptionStatus: true, subscriptionGraceUntil: true },
    })
    ownerPlan = owner?.plan || null
    ownerSubscriptionStatus = owner?.subscriptionStatus || null
    ownerSubscriptionGraceUntil = owner?.subscriptionGraceUntil || null
  }

  const state = resolveEffectiveEventAccessState({ billingTier: event.billingTier, ownerPlan, subscriptionStatus: ownerSubscriptionStatus, subscriptionGraceUntil: ownerSubscriptionGraceUntil })
...
export async function checkPhotoDownloadEntitlement(prisma, event) {
  ...
  const state = resolveEffectiveEventAccessState({
    ...
    subscriptionGraceUntil: ownerSubscriptionGraceUntil,
  })
