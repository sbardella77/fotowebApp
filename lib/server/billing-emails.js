...
const PROFESSIONAL_CANCELLATION_SCHEDULED = {
  de: {
    subject: 'Professional-Abonnement gekündigt',
    headline: 'Professional-Abonnement gekündigt',
    body: (periodEndDate, isAnnual, retentionUrl) => {
      const lines = [
        'Wir haben deine Kündigung erhalten.',
        `Dein Professional-Abonnement bleibt bis zum ${periodEndDate} aktiv.`,
        'Danach wird dein Account auf den Free-Plan zurückgesetzt. Deine Daten werden nicht sofort gelöscht.',
      ]
      if (!isAnnual) {
        lines.push('Wechsle jetzt zur jährlichen Abrechnung und spare 2 Monate.')
      }
      lines.push('Du kannst dein Abonnement jederzeit im Dashboard verwalten.')
      return lines
    },
    ctaDashboard: 'Zum Dashboard',
    ctaYearlyPlan: 'Jahresplan ansehen',
  },
  en: {
    subject: 'Professional subscription canceled',
    headline: 'Professional subscription canceled',
    body: (periodEndDate, isAnnual, retentionUrl) => {
      const lines = [
        'We received your cancellation request.',
        `Your Professional subscription remains active until ${periodEndDate}.`,
        'After that, your account will return to the Free plan. Your data will not be deleted immediately.',
      ]
      if (!isAnnual) {
        lines.push('Switch to yearly billing now and get 2 months free.')
      }
      lines.push('You can manage your subscription at any time from the dashboard.')
      return lines
    },
    ctaDashboard: 'Go to dashboard',
    ctaYearlyPlan: 'View yearly plan',
  },
  it: {
    subject: 'Abbonamento Professional annullato',
    headline: 'Abbonamento Professional annullato',
    body: (periodEndDate, isAnnual, retentionUrl) => {
      const lines = [
        'Abbiamo ricevuto la tua richiesta di annullamento.',
        `Il tuo abbonamento Professional resta attivo fino al ${periodEndDate}.`,
        'Dopo quella data il tuo account tornerà al piano Free. I tuoi dati non verranno eliminati immediatamente.',
      ]
      if (!isAnnual) {
        lines.push('Passa ora alla fatturazione annuale e ottieni 2 mesi gratis.')
      }
      lines.push('Puoi gestire il tuo abbonamento in qualsiasi momento dalla dashboard.')
      return lines
    },
    ctaDashboard: 'Vai alla dashboard',
    ctaYearlyPlan: 'Vedi piano annuale',
  },
  fr: {
    subject: 'Abonnement Professional annulé',
    headline: 'Abonnement Professional annulé',
    body: (periodEndDate, isAnnual, retentionUrl) => {
      const lines = [
        'Nous avons reçu votre demande d’annulation.',
        `Votre abonnement Professional reste actif jusqu’au ${periodEndDate}.`,
        'Après cette date, votre compte repassera au plan Free. Vos données ne seront pas supprimées immédiatement.',
      ]
      if (!isAnnual) {
        lines.push('Passez maintenant à la facturation annuelle et profitez de 2 mois offerts.')
      }
      lines.push('Vous pouvez gérer votre abonnement à tout moment depuis le tableau de bord.')
      return lines
    },
    ctaDashboard: 'Aller au tableau de bord',
    ctaYearlyPlan: 'Voir le plan annuel',
  },
  es: {
    subject: 'Suscripción Professional cancelada',
    headline: 'Suscripción Professional cancelada',
    body: (periodEndDate, isAnnual, retentionUrl) => {
      const lines = [
        'Hemos recibido tu solicitud de cancelación.',
        `Tu suscripción Professional permanecerá activa hasta el ${periodEndDate}.`,
        'Después de esa fecha, tu cuenta volverá al plan Free. Tus datos no se eliminarán de inmediato.',
      ]
      if (!isAnnual) {
        lines.push('Cambia ahora a la facturación anual y obtén 2 meses gratis.')
      }
      lines.push('Puedes gestionar tu suscripción en cualquier momento desde el panel.')
      return lines
    },
    ctaDashboard: 'Ir al panel',
    ctaYearlyPlan: 'Ver plan anual',
  },
}

export async function sendProfessionalCancellationScheduledEmail({ owner = {}, currentPeriodEnd = null, appUrl }) {
  const locale = resolveLocale(owner)
  const t = PROFESSIONAL_CANCELLATION_SCHEDULED[locale] || PROFESSIONAL_CANCELLATION_SCHEDULED[EMAIL_DEFAULT_LOCALE]
  const dashboardUrl = `${appUrl}/dashboard`
  const yearlyPlanUrl = `${appUrl}/pricing?plan=professional&billing=annual`
  const periodEndDate = currentPeriodEnd ? formatDate(currentPeriodEnd, locale) : null
  const isAnnual = owner.subscriptionBillingInterval === 'annual'

  const email = buildEmail({
    subject: t.subject,
    headline: t.headline,
    bodyLines: t.body(periodEndDate || '', isAnnual, yearlyPlanUrl),
    cta: {
      text: isAnnual ? t.ctaDashboard : t.ctaYearlyPlan,
      url: isAnnual ? dashboardUrl : yearlyPlanUrl,
    },
    locale,
    appUrl,
  })
  ...
  return sendEmailSafely({
    to: owner.email,
    email,
    logContext: { type: 'professional_cancellation_scheduled', ownerId: owner.id, currentPeriodEnd },
  })
}
