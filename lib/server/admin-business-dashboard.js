...
// Paid is deliberately NOT a funnel stage: it's a current snapshot (see
// countPaidAccounts() in admin-business-metrics.js), while every stage below
// is scoped to the selected date range. Mixing the two produced a
// mathematically-correct but semantically misleading conversion (e.g. a
// snapshot of 4 paid accounts against 1 period-scoped Core Activation showed
// as "400%"). Paid Accounts remains a separate KPI under `monetization`
// instead — see getAdminBusinessMetrics() below. Do not re-add it here
// without a reliable first-paid-timestamp/paid-conversion event to make it
