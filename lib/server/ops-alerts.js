...
import { sanitizeContext, redactMessage, serializeProviderError } from './safe-log'
...
export async function sendOpsAlert({ severity = 'warning', type, title, message, context = {} }) {
  ...
  // message is freeform text supplied by the caller (often a raw
  // dbError.message/err.message) — unlike context, it isn't a set of known
  // keys that can be stripped by name, so it goes through the same
  // email/URL/token scrubbing as any other logged error text.
  const safeMessage = message ? redactMessage(message) : undefined
  const logPayload = { type, title, message: safeMessage, ...safeContext }
  ...
  try {
    const result = await resend.emails.send({
      ...
      text: buildText({ severity, type, title, message: safeMessage, context: safeContext, appUrl }),
      html: buildHtml({ severity, type, title, message: safeMessage, context: safeContext, appUrl }),
    })

    console.log(`[ops-alert:${severity}:${type}] Alert email sent`, { id: result?.data?.id })
    return { sent: true, id: result?.data?.id }
  } catch (error) {
    console.error(`[ops-alert:${severity}:${type}] Failed to send alert email`, serializeProviderError('resend', 'send_ops_alert', error))
