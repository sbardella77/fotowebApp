 ...
 *
 * @param {string} email
 * @param {(owner) => boolean|Promise<boolean>} verifyFn — e.g. verifyPassword callback; result is always awaited
...
export async function findOwnerByEmailWithPassword(email, verifyFn) {
  ...
  const canonical = await resolveCanonicalOwner(email)
  if (canonical && canonical.passwordHash) {
    const valid = await verifyFn(canonical)
  ...
  for (const candidate of candidates) {
    if (candidate.passwordHash) {
      const valid = await verifyFn(candidate)
