...
export function normalizePublicPhoto(photo) {
  ...
  // url is either a non-empty string, or explicitly null/undefined — the
  // guest-safe DTO sends null for a photo with no safe derivative yet
  // (PENDING/FAILED/LEGACY_UNVERIFIED). That is a valid, expected state,
  // not malformed data: it renders as an explicit "unavailable" tile
  // (see photo-gallery-grid.jsx / photo-lightbox.jsx), never dropped and
  // never a reason to fall back to some other field. Any other falsy or
  // non-string value is still rejected as before.
  const rawUrl = photo.url
  let url = null
  if (rawUrl !== null && rawUrl !== undefined) {
    if (typeof rawUrl !== 'string' || rawUrl.length === 0) {
      logSkip(photo, 'missing-url', { urlType: typeof rawUrl })
      return null
    }
