...
function resolveAccountPlanBadge(plan) {
  if (plan === 'business') {
    return { plan: 'business', label: 'Business', variant: 'premium' }
  }
  if (plan === 'professional' || plan === 'pro') {
    return { plan: plan, label: plan === 'pro' ? 'Pro' : 'Professional', variant: 'premium' }
  }
  return { plan: 'free', label: 'Free', variant: 'free' }
}
...
export function resolveDashboardExperience({
  ...
  const { accountPremium } = resolveEffectiveEventAccessState({
    ownerPlan: plan,
    subscriptionStatus,
  ...
  const hasPremiumEvents = metrics?.hasPremiumEvents ?? events.some((e) => e.billingTier === 'pro_event' || e.billingTier === 'wedding_pro')
  const hasWeddingPro = metrics?.hasWeddingPro ?? events.some((e) => e.billingTier === 'wedding_pro')
  const hasProEvent = metrics?.hasProEvent ?? events.some((e) => e.billingTier === 'pro_event')
...
function resolveConsumerExperience({ plan, events, maturity, analyticsLevel, eventsCount, totalPhotos }) {
  return {
    ...
    planBadge: resolveAccountPlanBadge(plan),
    header: {
      titleKey: 'yourRooms',
      descriptionKey: 'yourRoomsDesc',
      ctaKey: 'createNewEvent',
    },
    insight: {
      showStats: true,
