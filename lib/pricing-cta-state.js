 ...
 * }}
 */
export function resolvePricingCtaState({ authenticated, intent, billingInterval = null }) {
  ...
  if (authenticated) {
    ...
    if (isProfessional) {
      const annualParam = billingInterval === 'annual' ? '&billing=annual' : ''
      return {
        labelKey: 'pricingStartProfessionalFromDashboard',
        href: `/dashboard?upgrade=professional&from=pricing${annualParam}`,
      }
    }
