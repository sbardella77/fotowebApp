...
export async function csrfFetch(url, options = {}) {
  ...
  if (mutable && isSameOrigin(url)) {
    try {
      ...
      // Logged in every environment (not just development): a request that
      // proceeds without a token is guaranteed to be rejected downstream
      // with a generic "Security check failed" message, so this is the only
      // place that records *why* the token fetch itself failed.
      // eslint-disable-next-line no-console
      console.warn('[csrfFetch] Failed to fetch CSRF token, request will be sent without one:', error.message)
    }
  }
